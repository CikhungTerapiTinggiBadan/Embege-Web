...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white h-20 flex justify-around items-center px-6 shadow-[0_-4px_20px_rgba(107,127,232,0.1)] z-50 rounded-t-[30px]">
      <Link href="/authed" className={`flex flex-col items-center ${isActive('/authed') ? 'text-[#6b7fe8]' : 'text-[#8b90b8]'}`}>
        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
          <path d="M3 9.5L12 3l9 6.5V20a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V9.5z"/>
        </svg>
        {isActive('/authed') && <div className="w-1 h-1 bg-[#6b7fe8] rounded-full mt-1"></div>}
      </Link>
      ...
      <Link href="/profil" className={`flex flex-col items-center ${isActive('/profile') ? 'text-[#6b7fe8]' : 'text-[#8b90b8]'}`}>
        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
